...
use crate::{
    config::get_config,
    types::{ActionResult, ActionType, ActionData, AppResult},
    utils,
};
...
impl ExecutionService {
    ...
    pub async fn execute(&self, action: &ActionResult) -> AppResult<bool> {
        utils::log_info(&format!("Executing action: {} ({})", action.title, action.id));
        
        match &action.action {
            ActionType::Launch { needs_terminal } => {
                self.execute_launch(action, *needs_terminal).await
            }
            ActionType::Navigate { path } => {
                self.execute_navigate(path).await
            }
            ...
            ActionType::Custom { action_id } => {
                self.execute_custom(action_id, action).await
            }
        }
    }
    ...
    async fn execute_launch(&self, action: &ActionResult, needs_terminal: bool) -> AppResult<bool> {
        let command = match &action.data {
            ...
            _ => return Err(crate::types::AppError::ActionExecution(
    ...
    async fn execute_navigate(&self, path: &str) -> AppResult<bool> {
        ...
        let command = format!("{} -d {} $SHELL", config.general.default_terminal, shell_safe_path);
