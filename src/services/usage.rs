...
impl UsageService {
    ...
    fn load(&mut self) -> Result<()> {
        ...
        let file = File::open(&self.file_path)
            .context("Failed to open usage stats file")?;
                ...
                match serde_json::from_str::<UsageEntry>(json_data) {
                    ...
                    Err(e) => {
                        crate::utils::log_warn(&format!(
                            "Failed to parse usage entry for '{}': {}", key, e
                        ));
                    }
                }
            }
    ...
    pub fn save(&mut self) -> Result<()> {
        ...
        if let Some(parent) = self.file_path.parent() {
            std::fs::create_dir_all(parent)
                .context("Failed to create usage stats directory")?;
        }
        ...
        for (key, entry) in sorted_entries {
            let json_data = serde_json::to_string(entry)
                .context("Failed to serialize usage entry")?;
    ...
    pub fn record_usage(&mut self, action_id: &str) {
        match self.entries.get_mut(action_id) {
            ...
            None => {
                self.entries.insert(action_id.to_string(), UsageEntry::new());
            }
        }
        self.dirty = true;
    }
...
pub fn init_usage_service() -> Result<()> {
    ...
    USAGE_SERVICE.set(Mutex::new(service))
...
pub fn get_top_used(limit: usize) -> Vec<String> {
    USAGE_SERVICE.get()
        .and_then(|service| service.lock().ok())
        .map(|service| service.get_top_used(limit)
            .into_iter()
            .map(|(id, _)| id.clone())
            .collect())
        .unwrap_or_default()
